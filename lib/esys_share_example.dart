...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _shareImage() async {
    try {
      final ByteData bytes = await rootBundle.load('assets/images/ss.jpg');
      await Share.file(
  ...
  Future<void> _shareImages() async {
    try {
      final ByteData bytes1 = await rootBundle.load('assets/images/ss.jpg');
      final ByteData bytes2 = await rootBundle.load('assets/images/ss.jpg');

      await Share.files(
        'esys images',
        {
          'ss.jpg': bytes1.buffer.asUint8List(),
          'sss.jpg': bytes2.buffer.asUint8List(),
        },
        {'image/png'},
      );
    } catch (e) {
  ...
  Future<void> _shareMixed() async {
    try {
      ...
      final ByteData bytes3 = await rootBundle.load('assets/images/ss.jpg');

      await Share.files(
          'esys images',
          {
            'esys.jpg': bytes1.buffer.asUint8List(),
            'bluedan.jpg': bytes2.buffer.asUint8List(),
            'addresses.jpg': bytes3.buffer.asUint8List(),
          },
          {'*/*'},
          text: 'My optional text.');
    } catch (e) {
